...
class Donate extends Command {
    async exec(args) {
        ...
        const {success, message} = await donate_currency({name, discord_id: interaction.user.id, guildId: interaction.guildId}, {guildId: interaction.guildId, discord_id: target_member.id, name: target_name}, donate_amount, interaction.botName);
