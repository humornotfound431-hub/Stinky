...
const battery_client = new Client({
    intents: [
        GatewayIntentBits.Guilds,
        GatewayIntentBits.GuildMessages,
        GatewayIntentBits.MessageContent
...
const setupBot = (botName, client, onInteraction) => {  
    const emotes = emote_map[botName]  
    ...
    client.on("interactionCreate", async (interaction) => {
        try {
            const interactionReply = await onInteraction(interaction, client);
            ...
            const sadEmote = botName === "garlic" ? emotes.crying : emotes.SadRheo;
            let msg = `Ah dang, some stoopid error happened ${sadEmote}`;
...
const garlic_interaction = async (interaction, client) => {
    ...
    if (interaction.isButton()) {
        ...
        switch (type) {
            ...
            case "daily":
                if (id === "claim") {
                    interactionReply = await Daily.claim(interaction, emote_map["garlic"], colors);
                }
                break;
            case "chess":
                if (chessInstance) {
        ...
        if (permSet && !permSet.has(`${interaction.channel.id}`)) {
            ...
            err.customMsg = `This command cannot be used here ${emote_map["garlic"].ha}`;
            throw err;
        }
        ...
        switch (cmd) {
            ...
            case "slots":
                ...
                break;
            case "cloves":
                interactionReply = await Currency.init({ interaction, name });
                break;
            case "donate":
...
const battery_interaction = async (interaction, client) => {
    const name = interaction.member?.displayName || interaction.user.globalName || interaction.user.username;
    let interactionReply;
    interaction.botName = "battery";
    ...
    if (interaction.isButton()) {
        ...
        if (permSet && !permSet.has(`${interaction.channel.id}`)) {
            const err = new Error("Used command in unauthorized space");
            err.customMsg = `This command cannot be used here ${emote_map["battery"].KAPPARHEO}`;
            throw err;
        }
...
setupBot("garlic", garlic_client, garlic_interaction);
setupBot("battery", battery_client, battery_interaction);

garlic_client.login(process.env.GARLIC_TOKEN);
battery_client.login(process.env.BATTERY_TOKEN);